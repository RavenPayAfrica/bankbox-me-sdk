...
class BankboxManager {
  ...
  private getTargetOrigin(params?: MountOptions): string {
    let url =
      this.environment === 'development'
        ...
        ? 'https://bankbox-me.netlify.app'
        : `https://${this.appName ?? 'bankly'}.bankbox.me`
    ...
    if (params?.streamViewPerCount) {
      queryParams.push(`streamViewPerCount=${params.streamViewPerCount}`)
    }
  ...
  private registerCoreListeners(config: Config): void {
    if (config.onSuccess)
      eventWorker.subscribe(this.constants.success, (e) =>
        config.onSuccess?.(e.detail)
      )
    if (config.onBluethoothConnected)
      eventWorker.subscribe(this.constants.bluethoothConnected, (e) =>
        config.onBluethoothConnected?.(e.detail)
      )
    if (config.onFail)
      eventWorker.subscribe(this.constants.fail, (e) =>
        config.onFail?.(e.detail)
      )
    if (config.onError)
      eventWorker.subscribe(this.constants.error, (e) =>
        config.onError?.(e.detail)
      )
    if (config.onLoad)
      eventWorker.subscribe(this.constants.load, () => config.onLoad?.())
  }
  ...
  private initWindowResizeListener(): void {
    ...
    window.addEventListener('resize', () => {
      ...
      if (this.container) {
        this.container.style.width = this.widgetOptions?.width
  ...
  private initializeListeners(): void {
    eventWorker.subscribe(
      this.constants.event_hook,
      this.handleIncomingMessage.bind(this)
    )
    eventWorker.subscribe(
      this.constants.bluethoothConnected,
      this.handleBluethoothConnected.bind(this)
    )
    eventWorker.subscribe(
      this.constants.systemReady,
      this.handleBankboxReady.bind(this)
    )
    ...
    if (!this.container) {
      ...
      if (!this.container) {
        ...
        this.container.style.width =
          this.windowSize.width > 900 ? '50%' : '100%'
        ...
        this.container.style.animation =
          'moveBankBoxMeInnerWrapUp 0.5s ease-in-out backwards'
  ...
  public mount(options?: MountOptions): void {
    if (options?.containerId) {
      ...
      if (!containerElement) {
        throw new Error(
          `Container element with id '${options?.containerId}' not found`
        )
      }
  ...
  public initPayment(options?: MountOptions): void {
    this.sendMessage({
      type: this.constants.sdkPaymentData,
      data: options,
      message: 'SDK Payment Data Recieved',
    })
  }
  public close(): void {
    if (this.container) {
